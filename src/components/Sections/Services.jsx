...
const Services = () => {
  ...
  return (
    <Section id='servicios' className='bg-slate-300 dark:bg-slate-800 dark:text-slate-100'>
      <Container>
        <TitleH2 title='Servicios' align={'items-center'} />
        {servicios.map((servicio) => {
          return (
            <article key={servicio.id} id={servicio.idService} className='flex flex-col mt-2 gap-32 w-full md:w-[100%] mx-auto place-items-center px-10 xl:w-[80%] xl:text-xl'>
              <div className='flex flex-col sm:flex-row gap-12'>
                <div className='flex flex-col gap-6 justify-center'>
                  ...
                  <p className='tracking-normal'>{servicio.serviceDescription}</p>
                </div>
