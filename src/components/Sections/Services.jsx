import { servicios } from '../../data'
import Section from '../Templates/Section'
import Container from '../Templates/Container'
import TitleH2 from '../Atoms/Headings/TitleH2'
import Lottie from 'lottie-react'


const Services = () => {
  return (
    <Section id='services' className='bg-slate-300 dark:bg-slate-800 dark:text-slate-100'>
      <Container>
        <TitleH2 title='Servicios' />
        <hr className='h-1 bg-gradient-to-r from-cyan-500 to-blue-500 w-[7%] skew-x-12 mb-20 mx-auto text-center mt-1' />
        {servicios.map((servicio) => {
          return (
            <article key={servicio.id} id={servicio.idService} className='flex flex-col mt-2 gap-32 w-full md:w-[100%] mx-auto place-items-center px-10 xl:w-[80%]'>
              <div className='flex flex-col sm:flex-row gap-12'>
                <div className='flex flex-col gap-6 justify-center'>
                  <h3 className='text-3xl font-semibold dark:text-blue-600'><span className='text-blue-700 dark:text-blue-500 mr-3 '>{servicio.id}.</span><span className='first-letter:text-blue-700'>{servicio.serviceName}</span></h3>
                  <p className='text-[1.3em] text-balance tracking-normal'>{servicio.serviceDescription}</p>
                </div>
                <div id={`lottie-${servicio.serviceName}`} className='xl:w-1/2 mb-12 sm:mb-0'>
                  <Lottie animationData={servicio.lottieImage} loop={true} className='w-[280px] my-9 sm:mr-0 mx-auto' />
                </div>
              </div>
            </article>

          )
        }
        )}
      </Container>
    </Section>
  )
}

export default Services