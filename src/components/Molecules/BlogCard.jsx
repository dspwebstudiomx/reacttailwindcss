  ...
  return (
    <Link to={`/blog/${id}`} key={id}>
      <article className='flex mx-auto border-1 border-gray-400 rounded-md shadow-xl animate__animated animate__flipInY flex-col w-[75%] sm:flex-col sm:w-[240px] xl:w-[220px] text-slate-900 dark:text-slate-900'>
        {/* Sección 1 */}
        <img src={imagen} alt='image' className='rounded-t-md w-full object-cover h-[160px] sm:h-[120px] xl:h-[120px] ' />
        {/* Bubble */}
        <div id={`Bubble-${id}`} className='absolute -top-2 -right-1 bg-gradient-to-r from-cyan-500 to-blue-500 px-4 py-2 text-white text-base shadow-lg rounded-bl-lg rounded-tr-lg border-2 border-blue-500 min-w-[120px] text-center'>{categoria}</div>
        {/* Bubble */}
        {/* Sección 1 */}

        {/* Sección 2 */}
        <div className='px-6 py-6 bg-slate-100 flex flex-col gap-1 rounded-b-md w-full min-h-[220px] sm:min-h-[210px] justify-center overflow-hidden'>
          <h3 className='text-xl font-semibold sm:text-lg xl:text-sm text-blue-800 leading-snug'>{titulo}</h3>
