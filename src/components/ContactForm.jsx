    ...
    <article id="formulario" className="border-2  border-cyan-600 rounded-md px-8 py-10 pb-14 bg-gradient-to-r from-slate-700 to-slate-900  text-slate-100 ">
      <form action="mailto:dspwebstudio@gmail.com" className="flex flex-col gap-6" method="POST" name="EmailForm">
        ...
        <div className="shield flex flex-col">
          ...
          <input type="text" name="phone" id="name" className="rounded-md text-slate-900 bg-slate-200 px-4 py-3 text-lg" />
        </div>
        <div className="shield flex flex-col">
          <label htmlFor="" className="mb-2 text-lg">Mensaje</label>
          <textarea type="text" name="message" id="name" className="rounded-md text-slate-900 bg-slate-200 p-4 min-h-40 max-h-40 text-lg" />
        </div>
